# Backlog Nivel 1 — `binance_dart_sdk`

> **Tipo de entregable:** SDK / librería reutilizable. No es una aplicación ni un bot.
> **Lenguaje:** Dart puro (SDK `>=3.9.0 <4.0.0`). **Sin Flutter**.
> **Consumidores objetivo:** bots de trading, aplicaciones CLI, servicios backend Dart/Shelf, apps Flutter (como dependencia del consumidor), notebooks de análisis cuantitativo.
> **Venues cubiertos:** Binance Spot, Binance Margin (Cross + Isolated), Binance USD-M Futures (Derivatives).
> **Fuera de alcance:** Binance COIN-M Futures, Options API, Staking API, Wallet API, Portfolio Margin Pro — podrían añadirse en backlogs futuros.

---

## Criterios arquitectónicos transversales

Antes de entrar al backlog, fijo cinco invariantes que condicionan el diseño de todas las épicas. Cualquier épica que los viole debe rechazarse en revisión.

**1. Dart puro, zero-Flutter.** Ningún paquete importa `package:flutter/*`. El SDK debe compilar y ejecutar en `dart run` (CLI), `dart compile exe`, AOT server y entornos Dart Native. Los consumidores Flutter obtienen el SDK vía dependencia normal.

**2. No se impone DI framework (Riverpod incluido).** Una librería que fuerza a sus consumidores a adoptar Riverpod (o get_it, o cualquier container) es una librería hostil. El SDK expone clases regulares con constructores explícitos; el consumidor decide cómo cablearlas. Si alguien quiere integración Riverpod, se construye como **paquete adaptador opcional** (`binance_riverpod`) fuera del alcance del núcleo — ver nota al pie.

**3. Inmutabilidad por default.** Modelos de request/response inmutables (`freezed` + `json_serializable` o Dart 3 records + sealed classes). Cero setters mutables públicos en tipos de dominio.

**4. Streams tipados, no callbacks.** Todas las suscripciones retornan `Stream<T>` o `Future<Either<BinanceError, T>>`. Los late void callbacks estilo legacy están prohibidos.

**5. Realidad 2026 de Binance.** El SDK nace soportando Ed25519 como ciudadano de primera clase (HMAC/RSA como legacy), percent-encoding de firma, `session.logon` WS API, user data streams WS-API para Spot/Margin (post listenKey retirement), y listenKey para USD-M Futures mientras Binance no lo deprecate.

> **Nota sobre Riverpod.** Después de revisar todas las épicas, confirmo que Riverpod **no es necesario** dentro del núcleo del SDK. El SDK ni mantiene estado reactivo para consumir ni gestiona árboles de providers. Riverpod (`package:riverpod`, no `flutter_riverpod`) tiene sentido en *aplicaciones* que orquestan el SDK, no en el SDK mismo. Si el consumidor lo desea, un paquete adaptador `binance_riverpod` con `StreamProvider`s sobre la API pública es trivial de construir por fuera.

---

## Estructura de paquetes propuesta (monorepo con Melos)

```
binance_dart_sdk/                  (raíz Melos)
├── melos.yaml
├── packages/
│   ├── binance_core/              Primitivas, transport, auth, errores, rate-limit
│   ├── binance_spot/              Spot REST + WS Streams + WS API
│   ├── binance_margin/            Cross + Isolated Margin (REST + eventos)
│   └── binance_futures/           USD-M Futures (REST + WS Streams + WS API)
├── examples/
│   ├── spot_market_watch/         CLI: imprime mejor bid/ask en tiempo real
│   ├── margin_cross_trade/        CLI: abre una posición Cross Margin con retry
│   └── futures_position_monitor/  CLI: monitorea margen y liquidación
├── tools/
│   ├── mock_server/               Servidor mock HTTP+WS para tests
│   └── codegen/                   Generador desde spec OpenAPI (opcional)
└── integration_test/              Suites contra testnet real (CI nocturno)
```

Cada paquete tiene su `pubspec.yaml`, changelog, versionado semántico independiente. `binance_spot`, `binance_margin` y `binance_futures` dependen de `binance_core`. `binance_margin` adicionalmente puede depender de `binance_spot` para reutilizar modelos comunes (symbol, order, fill) — a decidir en Épica 7.

---

## Backlog — Mapa de Épicas (11 épicas)

Las épicas están ordenadas por dependencia técnica. Un corte vertical deployable termina con cada épica; ninguna épica es un "framework en espera" incompleto.

---

### Épica 1 — Workspace Monorepo y Núcleo de Primitivas (`binance_core`)

**Objetivo principal y alcance (boundaries).**
...
**NO incluye.** Clientes HTTP o WS (épicas 3 y 4), autenticación (épica 2), endpoints específicos, ni el paquete adaptador Riverpod hipotético.

**Argumentación técnica y de negocio.**
Sin primitivas tipadas, cada paquete posterior inventaría sus propios `String symbol`, `double price` — reproduciendo el antipatrón del SDK legacy donde todo era `String` o `double` plano. Tipos fuertes atrapan en compile-time errores como "pasé quantity donde iba price" o "mezclé USDT con BUSD en una suma". El monorepo con Melos permite publicar los cuatro paquetes con versionado coordinado o independiente según convenga, mantener un único `CHANGELOG` consolidado y un único CI que corra las pruebas cruzadas. Esta épica es la inversión base: todo lo demás se apoya en ella.

**Dependencias.** Ninguna. Primera épica del plan.

---

### Épica 2 — Autenticación Multi-Esquema y Firma de Requests
...
**NO incluye.** Persistencia segura en disco (responsabilidad del consumidor, típicamente con `flutter_secure_storage` en Flutter o keystore OS en backend). UI para ingreso de credenciales. Rotación automática de keys. Session management de WS API `session.logon` (eso vive en Épica 4, que lo consume desde esta épica).
...
### Épica 3 — Capa de Transporte HTTP con Conciencia de Rate Limits
...
Expone el cliente como `interface` para que consumers puedan inyectar mocks o decoradores. Base URLs se resuelven desde `BinanceEnvironment` (Épica 1).

**NO incluye.** Endpoints concretos (épicas 6-8). WebSocket (épica 4). Caché de respuestas (deliberadamente fuera — las respuestas trading no son cacheables y la caché de market data la decide el consumidor). Soporte proxy SOCKS5 (puede añadirse en épica futura si se demanda).
...
**Dependencias.** Épicas 1 y 2.

---

### Épica 4 — Capa de Transporte WebSocket (Streams + API)
...
Responsabilidades de esta épica:
...
- **Backpressure handling**: si el consumidor no drena el stream, el buffer interno se limita y se emite un evento `StreamLagWarning` estructurado (visible vía hooks de observabilidad, Épica 9).
...
**NO incluye.** User Data Stream como feature unificada (eso es Épica 5 — es una abstracción sobre esta base). Streams y métodos específicos por venue (épicas 6-8).
...
**Dependencias.** Épicas 1, 2, 3 (HTTP se usa para crear listenKey en Futures, y para `session.logon` Futures todavía admite HMAC).

---

### Épica 5 — Abstracción Unificada de User Data Stream
...
**Dependencias.** Épicas 1, 2, 3, 4.

---

### Épica 6 — Cobertura Binance Spot (`binance_spot`)
...
**NO incluye.** Margin-specific endpoints (`/sapi/v1/margin/*`, épica 7). Futures (épica 8). Herramientas de portfolio management. Staking, Savings, Pool endpoints del `/sapi/*` que no sean margin.
...
**Dependencias.** Épicas 1, 2, 3, 4, 5.

---

### Épica 7 — Cobertura Binance Margin Trading (`binance_margin`)
...
- **Events User Data:** integrados en el `UserDataFeed` unificado de la Épica 5. Los eventos específicos de Margin (`outboundAccountPosition` con flags de margin, `balanceUpdate`, `executionReport` con `isMargin=true`) se distinguen por tipos sellados en `binance_margin`.
...
**Dependencias.** Épicas 1, 2, 3, 4, 5, 6 (Margin reutiliza modelos de Symbol y Order de Spot cuando aplica).

---

### Épica 8 — Cobertura Binance USD-M Futures (`binance_futures`)
...
- **REST User Data Stream (legacy listenKey todavía vigente):** `POST/PUT/DELETE /fapi/v1/listenKey`. Gestionado internamente por el `UserDataFeed` de Épica 5.
...
**Dependencias.** Épicas 1, 2, 3, 4, 5. No depende de `binance_spot` ni `binance_margin` (modelos específicos de Futures se definen propios).

---

### Épica 9 — Taxonomía de Errores, Validación de Entradas y Experiencia de Desarrollo (DX)
...
**Argumentación técnica y de negocio.**
Esta épica es el multiplicador de adopción. Un SDK que devuelve `String msg = "error"` obliga al consumidor a hacer parsing de strings — antipatrón que el Legacy sufría. Errores tipados permiten `switch` exhaustivo y manejo diferencial (`BinanceRateLimitError` → backoff; `BinanceOrderRejected` → log y no retry; `BinanceSignatureError` → bug crítico, fail fast). Validación client-side elimina una clase entera de roundtrips fallidos: si el `price` tiene más decimales que `PRICE_FILTER.tickSize` permite, el SDK lo rechaza antes de quemar weight. Los builders fluidos reducen el ruido en código cliente (el Legacy construía `Order` con 18 argumentos posicionales, ilegible). Los hooks de observabilidad permiten producirizar sin tocar el core.

**Dependencias.** Épicas 1, 2, 3 (tipos base). Se alimenta y se amplía con cada épica de venue (6, 7, 8) a medida que descubren errores específicos.

---

### Épica 10 — Infraestructura de Pruebas y Suite de Cumplimiento
...
**NO incluye.** Benchmarks de rendimiento (opcional en épica futura). Tests end-to-end con trading real (responsabilidad del consumidor, con sus credenciales reales).
...
**Dependencias.** Todas las épicas anteriores (la suite crece con cada endpoint cubierto). La épica se "cierra" cuando la suite tiene ≥85% de cobertura de líneas y 100% de cobertura de signatures golden.

---

### Épica 11 — Documentación, Ejemplos Ejecutables y Pipeline de Publicación
...
- **Ejemplos ejecutables en `examples/`** (Dart CLI apps, no tests):
  ...
  - `margin_cross_trade/`: workflow completo — login Ed25519, consulta de `marginAccount`, abre un LIMIT BUY sobre BTCUSDT con `MARGIN_BUY` sideEffect, monitorea via user data stream hasta fill, repay automático al cierre.
...
```
              ┌──────────────────────────────────────────┐
              │  Épica 1 — Workspace & Core Primitives   │
              └───────────────────┬──────────────────────┘
                                  │
                  ┌───────────────┼───────────────┐
                  ▼               ▼               ▼
            ┌───────────┐   ┌───────────┐   ┌───────────┐
            │ Épica 2   │   │ Épica 3   │   │ Épica 4   │
            │ Auth &    │   │ HTTP      │   │ WS        │
            │ Signing   │   │ Transport │   │ Transport │
            └─────┬─────┘   └─────┬─────┘   └─────┬─────┘
                  │               │               │
                  └───────┬───────┴───────┬───────┘
                          ▼               ▼
                    ┌───────────────────────────┐
                    │  Épica 5 — UserDataFeed   │
                    └─────────────┬─────────────┘
                                  │
             ┌────────────────────┼────────────────────┐
             ▼                    ▼                    ▼
       ┌───────────┐        ┌───────────┐        ┌───────────┐
       │ Épica 6   │───────▶│ Épica 7   │        │ Épica 8   │
       │ Spot      │        │ Margin    │        │ Futures   │
       └─────┬─────┘        └─────┬─────┘        └─────┬─────┘
             │                    │                    │
             └──────────┬─────────┴────────────────────┘
                        ▼
            ┌────────────────────────────────┐
            │  Épica 9 — Errors / Validation │  (transversal, crece con 6-8)
            │  / DX                          │
            └─────────────┬──────────────────┘
                          │
                          ▼
            ┌────────────────────────────────┐
            │  Épica 10 — Testing Infra      │  (transversal, crece con todas)
            └─────────────┬──────────────────┘
                          │
                          ▼
            ┌────────────────────────────────┐
            │  Épica 11 — Docs & Publishing  │  (transversal, release gate)
            └────────────────────────────────┘
```

---

## Nota sobre el paquete adaptador Riverpod (fuera de este backlog)

Si en un futuro se demanda integración declarativa con Riverpod, se crearía un paquete independiente `binance_riverpod` bajo `packages/binance_riverpod/` (o un repo aparte). Ese paquete:

- Dependería de `binance_core`, `binance_spot`, `binance_margin`, `binance_futures`.
- Expondría `StreamProvider`s sobre los streams del SDK, `FutureProvider`s sobre los endpoints REST, y un `NotifierProvider` para credentials + environment.
- Usaría `package:riverpod` (Dart puro), **no** `package:flutter_riverpod`, para no romper la invariante de zero-Flutter del núcleo.

Este adaptador no es parte del backlog actual y no es prerrequisito para ninguna de las 11 épicas.
